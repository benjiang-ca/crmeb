...
<script>
import {
  formatLstApi, attachmentCreateApi, attachmentUpdateApi, attachmentDeleteApi, attachmentListApi, picDeleteApi, categoryApi
} from '@/api/system'
...
export default {
  ...
  methods: {
    ...
    handleDelete(id) {
      this.$modalSure().then(() => {
        attachmentDeleteApi(id).then(({ message }) => {
          this.$message.success(message)
          this.getList()
        }).catch(({ message }) => {
    ...
    getFileList() {
      this.loading = true
      attachmentListApi(this.tableData).then(async res => {
    ...
    getMove() {
      categoryApi(this.ids, this.sleOptions.attachment_category_id).then(async res => {
